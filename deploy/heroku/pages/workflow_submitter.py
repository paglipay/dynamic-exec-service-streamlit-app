...
def _post_execute(api_base: str, method: str, args: list) -> dict:
    ...
    if body.get("status") != "success":
        raise RuntimeError(body.get("message", "Service returned an error"))
...
if not st.session_state.get(load_key):
    try:
        # For /execute templates inside subdirs, ask the catalog to look in
        # "execute/<subdir>" by passing a custom category map via constructor.
        # The plugin's `name` field rejects path separators, so we point its
        # category dir at the subdir for this call.
        if subdir:
            payload = {
                "module": CATALOG_MODULE,
                "class": CATALOG_CLASS,
                ...
                "constructor_args": {
                    "categories": {"_scoped": f"jsons/system_tools/{subdir}"},
                },
                "args": [{"category": "_scoped", "name": leaf}],
            }
            r = requests.post(f"{api_base.rstrip('/')}/execute", json=payload, timeout=15)
            r.raise_for_status()
            body = r.json()
            if body.get("status") != "success":
                raise RuntimeError(body.get("message", "service error"))
